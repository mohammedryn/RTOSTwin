...
/* ── Idle-tick counters for CPU utilisation  ─────────────────────── */
static volatile uint32_t s_idle_ticks   = 0U;
...
void snapshot_capture(full_snapshot_t *out) {
    ...
    /* 1. Timestamp — read before entering critical section */
...
/**
 * This function is called automatically by FreeRTOS every time the CPU
 * has nothing useful to do (i.e., all other tasks are blocked/sleeping).
 * We count how often this happens to derive CPU utilisation.
 */
